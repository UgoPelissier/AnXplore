...
def compute_indicators(
    ...
    # Load the mesh
    path = osp.join(data_dir, "vtu", filename)
    ...
    indicators = [min_wss, max_wss, min_wss_aneurysm, max_wss_aneurysm, min_wss_vessels, max_wss_vessels, mean_wss, std_wss, mean_wss_aneurysm, std_wss_aneurysm, mean_wss_vessels, std_wss_vessels, mean_osi_aneurysm, max_osi_aneurysm, KER, VDR, LSA, HSA, SCI, ICI]
...
if __name__ == '__main__':
    ...
    # Loop over vtu files in data/vtu/

    os.makedirs(osp.join(data_dir, "csv"), exist_ok=True)
    indicators = []
    cardiac_cycle = sorted(os.listdir(osp.join(data_dir, "vtu")))[-3:]
